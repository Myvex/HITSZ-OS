...
bget(uint dev, uint blockno)
{
  ...
  for(b = bcache.hashbucket[no].next; b != &bcache.hashbucket[no]; b = b->next){
  ...
  while(i != no)
  {
    ...
    for(b = bcache.hashbucket[i].next; b != &bcache.hashbucket[i]; b = b->next){
      if(b->refcnt == 0) {
        ...
        b->next->prev = b->prev;
        // acquire(&bcache.lock[no]);
        b->next = bcache.hashbucket[no].next;
        b->prev = &bcache.hashbucket[no];
        bcache.hashbucket[no].next->prev = b;
        bcache.hashbucket[no].next = b;
